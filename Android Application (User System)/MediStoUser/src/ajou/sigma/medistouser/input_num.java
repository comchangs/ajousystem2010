...
public class input_num extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		//입고
			public void onClick(View v) {
				...
				for(int i=0; i<12; i++) {
					ajou.sigma.medistouser.input.parsedata[i]="";
				}
				
				if(itemID.getText().toString().equals("")) {
					Toast.makeText(input_num.this, "관리번호를 입력하세요.", Toast.LENGTH_SHORT).show();
				} else {
				ajou.sigma.medistouser.input.num = itemID.getText().toString();
				//초기화
				...
				try{
				        ...
				        if(ajou.sigma.medistouser.input.parsedata[1]=="") {
				        	Toast.makeText(input_num.this, "데이터가 없습니다.", Toast.LENGTH_SHORT).show();
				        } else {
						if(Integer.parseInt(login.level)<=Integer.parseInt(ajou.sigma.medistouser.input.parsedata[2])) {
				        	Intent intent = new Intent (input_num.this, item_increase.class);
				        	startActivity(intent);
				        } else {
				        	Toast.makeText(input_num.this, "허가 거부되었습니다.", Toast.LENGTH_SHORT).show();
				        }
				        }
				}
			}
		...
		buttonNum0.setOnClickListener(new Button.OnClickListener(){
			...
			public void onClick(View v) {
				if (itemID.length()<4) {
					...
					Toast.makeText(input_num.this, "관리번호는 최대 4글자 입니다.", Toast.LENGTH_SHORT).show();
				}
