...
public class output_code extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.output_code);
		
		System.loadLibrary("MediStoUser");

		ajou.sigma.medistouser.hwcontrol.CISCameraIOctl(0x0);
		iv = (ImageView)findViewById(R.id.out_image);
		bt = (Button)findViewById(R.id.out_Button01);
		width = 320; 
		height = 240;  
	...
	private final Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {           
			...
			case START_PREVIEW:
				...
				if(rValue < 90 && gValue < 90 && bValue < 90){
			...
			case DECODE:
				...
				{
					Toast.makeText(output_code.this, "barcode decoding error", Toast.LENGTH_SHORT).show();
					mHandler.sendEmptyMessage(START_PREVIEW);
				}
				else{
					...
					for(int i1=0; i1<12; i1++) {
						ajou.sigma.medistouser.output.parsedata[i1]="";
					}

					Toast.makeText(output_code.this, "barcode : " + Integer.toString(decode_bar), Toast.LENGTH_SHORT).show();
					ajou.sigma.medistouser.output.num = Integer.toString(decode_bar);
					parsing_barcode();

					if(ajou.sigma.medistouser.output.parsedata[2].equals("")) {
						mHandler.sendEmptyMessage(START_PREVIEW);
						Toast.makeText(output_code.this, Integer.toString(decode_bar) + " : 해당 품목이 없습니다.", Toast.LENGTH_SHORT).show();
					} else {
					if(Integer.parseInt(login.level)<=Integer.parseInt(ajou.sigma.medistouser.output.parsedata[2])) {
						Intent intent = new Intent (output_code.this, item_increase.class);
						startActivity(intent);
					} else {
						Toast.makeText(output_code.this, "허가 거부되었습니다.", Toast.LENGTH_SHORT).show();
					}
					}
				}
